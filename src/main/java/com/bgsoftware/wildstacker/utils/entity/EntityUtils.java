...
public final class EntityUtils {
    ...
    private static final ReflectMethod<Entity> GET_SHOULDER_ENTITY_RIGHT = new ReflectMethod<>(
        HumanEntity.class, "getShoulderEntityRight");
    private static final ReflectMethod<Void> SET_SHOULDER_ENTITY_RIGHT = new ReflectMethod<>(
        HumanEntity.class, "setShoulderEntityRight", Entity.class);
    private static final ReflectMethod<Entity> GET_SHOULDER_ENTITY_LEFT = new ReflectMethod<>(
        HumanEntity.class, "getShoulderEntityLeft");
    private static final ReflectMethod<Void> SET_SHOULDER_ENTITY_LEFT = new ReflectMethod<>(
        HumanEntity.class, "setShoulderEntityLeft", Entity.class);

    private static final WildStackerPlugin plugin = WildStackerPlugin.getPlugin();
    private static final Object NULL = new Object();
    private static final Enchantment CURSE_OF_VANISH = Arrays.stream(Enchantment.values())
        .filter(enchantment -> enchantment.getName() != null && enchantment.getName().equals(
            "VANISHING_CURSE"))
    ...
    public static String format(String type) {
        ...
        for (String section : type.split("_")) {
            name.append(section.substring(0, 1).toUpperCase()).append(
                section.substring(1).toLowerCase()).append(" ");
        }
    ...
    public static void removeParrotIfShoulder(Parrot parrot) {
        if (GET_SHOULDER_ENTITY_RIGHT.isValid()) {
            Collection<Entity> entities = EntitiesGetter.getNearbyEntities(
                ((Animals) parrot).getLocation(), 1, entity -> entity instanceof Player);
    ...
    public static String getEntityName(StackedEntity stackedEntity) {
        ...
        if (stackedEntity.getCustomName() != null) {
            if (stackedEntity.getSpawnCause() == SpawnCause.MYTHIC_MOBS) {
                return MythicMobsHook.getMythicName(stackedEntity.getLivingEntity()).replace("{}",
                                                                                             String.valueOf(
                                                                                                 stackAmount));
            } else if (PluginHooks.isLevelledMobsEnabled && LevelledMobsHook.isLevelledMob(
                stackedEntity.getLivingEntity())) {
                return plugin.getNMSAdapter().getCustomName(
                    stackedEntity.getLivingEntity()).replace("{}", String.valueOf(stackAmount));
            }
        }
    ...
    public static StackCheckResult areSimilar(Entity en1, Entity en2) {
        ...
        if (StackCheck.ANIMAL_OWNER.isEnabled() && en1 instanceof Tameable) {
            if (!requireNotNull(((Tameable) en1).getOwner()).equals(
                requireNotNull(((Tameable) en2).getOwner()))) {
                return StackCheckResult.ANIMAL_OWNER;
            }
        }

        if (StackCheck.SKELETON_TYPE.isEnabled() && StackCheck.SKELETON_TYPE.isTypeAllowed(
            entityType)) {
            try {
        ...
        if (StackCheck.ZOMBIE_BABY.isEnabled() && StackCheck.ZOMBIE_BABY.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.ZOMBIE_PIGMAN_ANGRY.isEnabled() && StackCheck.ZOMBIE_PIGMAN_ANGRY.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.ENDERMAN_CARRIED_BLOCK.isEnabled() && StackCheck.ENDERMAN_CARRIED_BLOCK.isTypeAllowed(
            entityType)) {
            if (!plugin.getNMSAdapter().getEndermanCarried((Enderman) en1).equals(
                plugin.getNMSAdapter().getEndermanCarried((Enderman) en2))) {
        ...
        if (StackCheck.GUARDIAN_ELDER.isEnabled() && StackCheck.GUARDIAN_ELDER.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.SHEEP_SHEARED.isEnabled() && StackCheck.SHEEP_SHEARED.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.SHEEP_COLOR.isEnabled() && StackCheck.SHEEP_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.WOLF_COLLAR_COLOR.isEnabled() && StackCheck.WOLF_COLLAR_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.OCELOT_TYPE.isEnabled() && StackCheck.OCELOT_TYPE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.CAT_COLLAR_COLOR.isEnabled() && StackCheck.CAT_COLLAR_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.HORSE_TAME_PROGRESS.isEnabled() && StackCheck.HORSE_TAME_PROGRESS.isTypeAllowed(
            entityType)) {
            if (en1 instanceof Horse) {
        ...
        if (StackCheck.HORSE_MAX_TAME_PROGRESS.isEnabled() && StackCheck.HORSE_MAX_TAME_PROGRESS.isTypeAllowed(
            entityType)) {
            if (en1 instanceof Horse) {
        ...
        if (StackCheck.RABBIT_TYPE.isEnabled() && StackCheck.RABBIT_TYPE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.VILLAGER_PROFESSION.isEnabled() && StackCheck.VILLAGER_PROFESSION.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.LLAMA_COLOR.isEnabled() && StackCheck.LLAMA_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.LLAMA_STRENGTH.isEnabled() && StackCheck.LLAMA_STRENGTH.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.PARROT_TYPE.isEnabled() && StackCheck.PARROT_TYPE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.PUFFERFISH_STATE.isEnabled() && StackCheck.PUFFERFISH_STATE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.TROPICALFISH_TYPE.isEnabled() && StackCheck.TROPICALFISH_TYPE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.TROPICALFISH_BODY_COLOR.isEnabled() && StackCheck.TROPICALFISH_BODY_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.TROPICALFISH_TYPE_COLOR.isEnabled() && StackCheck.TROPICALFISH_TYPE_COLOR.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.PHANTOM_SIZE.isEnabled() && StackCheck.PHANTOM_SIZE.isTypeAllowed(
            entityType)) {
        ...
        if (StackCheck.MOOSHROOM_TYPE.isEnabled() && StackCheck.MOOSHROOM_TYPE.isTypeAllowed(
            entityType)) {
            if (plugin.getNMSAdapter().getMooshroomType(
                (MushroomCow) en1) != plugin.getNMSAdapter().getMooshroomType((MushroomCow) en2)) {
...
//    }
    ...
    public static List<ItemStack> getEquipment(LivingEntity livingEntity, int lootBonusLevel) {
        ...
        if (ServerVersion.isAtLeast(ServerVersion.v1_9)) {
            addDropArmor(drops, livingEntity,
                         plugin.getNMSAdapter().getItemInOffHand(entityEquipment), lootBonusLevel,
                         plugin.getNMSAdapter().getItemInOffHandDropChance(entityEquipment));
        }

        addDropArmor(drops, livingEntity, entityEquipment.getHelmet(), lootBonusLevel,
                     entityEquipment.getHelmetDropChance());
        addDropArmor(drops, livingEntity, entityEquipment.getChestplate(), lootBonusLevel,
                     entityEquipment.getChestplateDropChance());
        addDropArmor(drops, livingEntity, entityEquipment.getLeggings(), lootBonusLevel,
                     entityEquipment.getLeggingsDropChance());
        addDropArmor(drops, livingEntity, entityEquipment.getBoots(), lootBonusLevel,
                     entityEquipment.getBootsDropChance());
        ...
        switch (EntityTypes.fromEntity(livingEntity)) {
            case PIG:
                ...
                break;
            case STRIDER:
                if (plugin.getNMSAdapter().doesStriderHaveSaddle(
                    (org.bukkit.entity.Strider) livingEntity)) {
                    drops.add(new ItemStack(Material.SADDLE));
                }
                break;
            case HORSE:
            ...
            case TRADER_LLAMA: {
                ...
                for (ItemStack itemStack : inventory.getContents()) {
                    if (itemStack != null && itemStack.getType() != Material.AIR &&
                        (CURSE_OF_VANISH == null || itemStack.getEnchantmentLevel(
                            CURSE_OF_VANISH) <= 0)) {
                        drops.add(itemStack);
                    }
                }
    ...
    public static void clearEquipment(LivingEntity livingEntity) {
        ...
        if (clearEquipment || plugin.getNMSAdapter().getItemInMainHandDropChance(
            entityEquipment) >= 2.0F) {
            plugin.getNMSAdapter().setItemInMainHand(entityEquipment, new ItemStack(Material.AIR));
        }

        if (ServerVersion.isAtLeast(
            ServerVersion.v1_9) && (clearEquipment || plugin.getNMSAdapter().getItemInOffHandDropChance(
            entityEquipment) >= 2.0F)) {
        ...
        switch (EntityTypes.fromEntity(livingEntity)) {
            case PIG:
                ...
                break;
            case STRIDER:
                plugin.getNMSAdapter().removeStriderSaddle(
                    (org.bukkit.entity.Strider) livingEntity);
                break;
            case HORSE:
            case DONKEY:
    ...
    private static void addDropArmor(List<ItemStack> drops, LivingEntity livingEntity, ItemStack itemStack, int lootBonusLevel, double dropChance) {
        ...
        if (itemStack != null && itemStack.getType() != Material.AIR &&
            ...
            if (dropChance <= 1 && plugin.getNMSAdapter().shouldArmorBeDamaged(itemStack)) {
                ...
                toDrop.setDurability((short) (maxDurability - random.nextInt(
                    1 + random.nextInt(Math.max(maxDurability - maxDur, 1)))));
            }
